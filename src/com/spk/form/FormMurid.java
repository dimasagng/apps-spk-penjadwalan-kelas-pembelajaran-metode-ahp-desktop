...
import com.spk.form.input.FormInputMurid;
import com.spk.main.Form;
import com.spk.model.Murid;
import com.spk.tablemodel.TabModMurid;
...
public class FormMurid extends Form {

    private final ServiceMurid servis = new MuridDAO();
    private final TabModMurid tblModel = new TabModMurid();
    
    private final JTable tblData = new JTable();
    private JTextField txtSearch;

    private int idMuridFromTable;
    ...
    public FormMurid() {
        ...
        private JPanel setInfo() {
            ...
            JLabel lbTitle = new JLabel("Data Murid");
    ...
    private void setTableProperties() {
        TabelUtils.setColumnWidths(tblData, 
                new int[]{0, 2, 3, 4, 5, 6}, 
                new int[]{80, 150, 200, 120, 80, 100}); // <-- Lebar kolom 2 dan 3 diperbesar
    ...
    private void loadData() {
        List<Murid> list = servis.getData();
    ...
    private void searchData() {
        ...
        List<Murid> list = servis.searchData(keyword);
        tblModel.setData(list);
        tblData.setModel(tblModel);
    }
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this,
                new SimpleModalBorder(new FormInputMurid(null, this),
                        "Add Murid"), option, "Form Input");
    }
    
    private void updateData() {
    int row = tblData.getSelectedRow();
    if (row != -1) {
        Murid model = tblModel.getData(row);
        Option option = ModalDialog.createOption();
        ModalDialog.showModal(this,
                new SimpleModalBorder(new FormInputMurid(model, this),
                        "Update Murid"), option, "Form Update");
    } else {
    ...
    private void deleteData() {
        ...
        if (row != -1) {
            Murid model = tblModel.getData(row);
            idMuridFromTable = model.getIdMurid();
                    ...
                    new MessageModal(
                            ...
                            (controller, action) -> {
                                if (action == SimpleModalBorder.YES_OPTION) {
                                    ...
                                    // Set INT id_murid untuk dikirim ke DAO
                                    modelMurid.setIdMurid(idMuridFromTable); 

                                    servis.deleteData(modelMurid);
